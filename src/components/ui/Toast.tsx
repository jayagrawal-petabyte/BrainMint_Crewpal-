...
// ─── Toast Icon & Styling Map ────────────────────────────────────────────────

const TOAST_CONFIG: Record<ToastType, { icon: typeof CheckCircle2; bg: string; border: string; text: string }> = {
  success: { icon: CheckCircle2, bg: 'bg-olive-100', border: 'border-olive-300', text: 'text-olive-800' },
  error:   { icon: AlertCircle,  bg: 'bg-rose-100',  border: 'border-rose-300',  text: 'text-rose-800' },
  warning: { icon: AlertTriangle, bg: 'bg-cream-200', border: 'border-cream-300', text: 'text-forest-800' },
  info:    { icon: Info,          bg: 'bg-teal-100',  border: 'border-teal-300',  text: 'text-teal-800' },
};
        ...
        return (
          ...
          >
            ...
            <button
              ...
              className={`shrink-0 p-0.5 rounded-full hover:bg-black/5 transition-colors ${config.text}`}
            >
